...
void GxGDEW042Z15::updateWindow(uint16_t x, uint16_t y, uint16_t w, uint16_t h, bool usingRotation) {
  uint16_t xe = min(GxGDEW042Z15_WIDTH, x + w) - 1;
  uint16_t ye = min(GxGDEW042Z15_HEIGHT, y + h) - 1;
  uint16_t xs_bx = x / 8;
  uint16_t xe_bx = (xe + 7) / 8;

  _wakeUp();
  _writeCommand(PARTIAL_IN);
  _writeCommand(PARTIAL_WINDOW);
  ...
  _writeData(ye % 256);
  // _writeData(x >> 8);
  // _writeData(x & 0xf8); // x should be the multiple of 8, the last 3 bit will always be ignored
  // _writeData(((x & 0xf8) + w  - 1) >> 8);
  // _writeData(((x & 0xf8) + w  - 1) | 0x07);
  // _writeData(y >> 8);
  // _writeData(y & 0xff);
  // _writeData((y + h - 1) >> 8);
  ...
  _writeCommand(DISPLAY_REFRESH);
  _waitWhileBusy("updateWindow");
  _writeCommand(PARTIAL_OUT);
}
...
void GxGDEW042Z15::update(void)
{
  ...
  _writeCommand(DISPLAY_REFRESH); //display refresh
...
void GxGDEW042Z15::_sleep(void)
{
  ...
  _writeData(0xF7);
  _writeCommand(POWER_OFF); // power off
  _waitWhileBusy("Power Off");
  _writeCommand(DEEP_SLEEP); // deep sleep
  _writeData(0xA5);
}
